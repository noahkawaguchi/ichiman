...
def update_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
    ...
    if latest_date >= today:
        ...
        # Display the date for which the user should enter data
        with c2:
        ...
        # get valid return values.
        up_to_date = False
        return st.session_state.new_df, up_to_date

def up_to_date_download(df: pd.DataFrame) -> None:
    """Format the updated DataFrame and create a CSV file.
    ...
    with c1:
        ...
        st.write(f'#### This data is up to date! ({latest_date.strftime('%B %e')})')
    with c2:
        c1, c2 = st.columns(2)
        with c1:
